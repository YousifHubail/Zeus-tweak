...
static inline BOOL zeus_liquidGlassSurfacesWanted(void) {
    return ENABLED(@"Enable Liquid Glass Surfaces");
}

/** Homecoming experiment bypass: either LG toggle. */
static inline BOOL zeus_liquidGlassFloatingBarWanted(void) {
...
static BOOL hook_IGFloatingTabBarEnabled(void) {
    if (zeus_liquidGlassSurfacesWanted()) return YES;
...
static NSInteger hook_IGTabBarStyleForLauncherSet(NSInteger set) {
    if (zeus_liquidGlassSurfacesWanted()) return 1;
    return orig_IGTabBarStyleForLauncherSet ? orig_IGTabBarStyleForLauncherSet(set) : set;
}

static BOOL zeus_tryInstallLiquidGlassTabBarCSymbolHooks(void) {
#ifdef SIDELOAD
    // These are plain C functions living inside FBSharedFramework, so hooking
    // them means patching instructions in a signed executable page -- which
    // cannot work jailed (see ZeusMSHookFunction). IGFloatingTabBarEnabled and
    // IGTabBarStyleForLauncherSet happen to share one 16K page, so patching
    // either one took the whole page non-executable and killed the app during
    // startup. The ObjC-level Liquid Glass hooks above still apply normally.
    return NO;
#endif
    ...
    if (!ZeusSubstrateLoad()) return NO;

    if (!floatingDone) {
        void *sym = ZeusResolveInstagramExecutableSymbol("IGFloatingTabBarEnabled");
        if (sym) {
            ZeusMSHookFunction(sym, (void *)hook_IGFloatingTabBarEnabled, (void **)&orig_IGFloatingTabBarEnabled);
            floatingDone = YES;
        }
    }
    if (!dynamicDone) {
        void *sym = ZeusResolveInstagramExecutableSymbol("IGTabBarDynamicSizingEnabled");
        if (sym) {
            ZeusMSHookFunction(sym, (void *)hook_IGTabBarDynamicSizingEnabled, (void **)&orig_IGTabBarDynamicSizingEnabled);
            dynamicDone = YES;
        }
    }
    if (!enhancedDone) {
        void *sym = ZeusResolveInstagramExecutableSymbol("IGTabBarEnhancedDynamicSizingEnabled");
        if (sym) {
            ZeusMSHookFunction(sym, (void *)hook_IGTabBarEnhancedDynamicSizingEnabled, (void **)&orig_IGTabBarEnhancedDynamicSizingEnabled);
            enhancedDone = YES;
        }
    }
    if (!homecomingDone) {
        void *sym = ZeusResolveInstagramExecutableSymbol("IGTabBarHomecomingWithFloatingTabEnabled");
        if (sym) {
            ZeusMSHookFunction(sym, (void *)hook_IGTabBarHomecomingWithFloatingTabEnabled, (void **)&orig_IGTabBarHomecomingWithFloatingTabEnabled);
            homecomingDone = YES;
        }
    }
    if (!viewpointDone) {
        void *sym = ZeusResolveInstagramExecutableSymbol("IGTabBarViewPointFixEnabled");
        if (sym) {
            ZeusMSHookFunction(sym, (void *)hook_IGTabBarViewPointFixEnabled, (void **)&orig_IGTabBarViewPointFixEnabled);
            viewpointDone = YES;
        }
    }
    if (!styleDone) {
        void *sym = ZeusResolveInstagramExecutableSymbol("IGTabBarStyleForLauncherSet");
        if (sym) {
            ZeusMSHookFunction(sym, (void *)hook_IGTabBarStyleForLauncherSet, (void **)&orig_IGTabBarStyleForLauncherSet);
            styleDone = YES;
        }
    }
...
static NSString *zeus_expName(id obj) {
    if (!obj) return nil;
...
static inline BOOL zeus_isHomecomingExp(NSString *name) {
...
static id hook_meta_groupName(id self, SEL _cmd) {
    if (zeus_liquidGlassFloatingBarWanted() && zeus_isHomecomingExp(zeus_expName(self)))
        return @"test";
...
static BOOL hook_meta_isInExp(id self, SEL _cmd) {
    if (zeus_liquidGlassFloatingBarWanted() && zeus_isHomecomingExp(zeus_expName(self))) return YES;
...
static BOOL hook_lid_isExpEnabled(id self, SEL _cmd, NSString *name) {
    if (zeus_liquidGlassFloatingBarWanted() && zeus_isHomecomingExp(name)) return YES;
    return orig_lid_isExpEnabled ? orig_lid_isExpEnabled(self, _cmd, name) : NO;
}

static Class zeus_resolveIGNavConfigurationClass(void) {
...
static BOOL hook_navCfg_isHomecoming(id self, SEL _cmd) {
    if (zeus_liquidGlassFloatingBarWanted()) return YES;
    return orig_navCfg_isHomecoming ? orig_navCfg_isHomecoming(self, _cmd) : NO;
}

// ── Homecoming UI hooks (installed from ZURegisterLiquidGlassHooks) ───────────
...
static id hook_surfaceIntent_resolved(id self, SEL _cmd, BOOL enabled) {
    if (zeus_liquidGlassFloatingBarWanted()) enabled = YES;
...
static void zeus_tryInstallLiquidGlassExperimentHooksOnce(void) {
    ...
    if (!navDone) {
        Class navCfg = zeus_resolveIGNavConfigurationClass();
...
// repeat attempts on main + when ZURegisterLiquidGlassHooks runs.
void ZURegisterLiquidGlassTabBarEarlyHooks(void) {
    zeus_tryInstallLiquidGlassTabBarCSymbolHooks();
    zeus_tryInstallLiquidGlassExperimentHooksOnce();
...
void ZURegisterLiquidGlassHooks(void) {
    zeus_tryInstallLiquidGlassTabBarCSymbolHooks();
    zeus_tryInstallLiquidGlassExperimentHooksOnce();
