	...
	if (self) {
		...
		if (IGDSSwitchClass) {
			...
			if ([self.embeddedSwitch respondsToSelector:@selector(setOnTintColor:)]) {
				[(id)self.embeddedSwitch performSelector:@selector(setOnTintColor:) withObject:[ZeusHelper iotaPinkColor]];
			}
		} else {
            NSLog(@"IGDSSwitchClass not found");
			self.embeddedSwitch = [[UISwitch alloc] initWithFrame:CGRectZero];
			((UISwitch *)self.embeddedSwitch).onTintColor = [ZeusHelper iotaPinkColor];
		}
