...
static UIViewController *ZeusPresenterForAlert(void) {
...
static void PresentAggregatedHookFailureAlert(void) {
    ...
    NSString *title = [NSString stringWithFormat:@"Zeus Hook Errors (%lu)", (unsigned long)n];
    ...
    UIViewController *root = ZeusPresenterForAlert();
    if (root) {
        [root presentViewController:alert animated:YES completion:nil];
    } else {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            UIViewController *retry = ZeusPresenterForAlert();
            if (retry) {
...
static id ZeusValueForKey(id obj, NSString *key) {
...
static void ZeusSetValueForKey(id obj, id value, NSString *key) {
...
static void ZeusSetCaptureHiding(UIView *view) {
    if (!view) return;
    if (ENABLED(@"Hide Zeus From Screenshots")) {
        ZeusSetValueForKey(view.layer, @((1 << 1) | (1 << 4)), @"disableUpdateMask");
    }
}
 ...
 */
static BOOL ZeusInstallMessageHook(Class cls, SEL sel, void *replacement, void *original, BOOL reportMissing) {
    if (!cls || !replacement) {
...
static void NullHookMessageEx(Class cls, SEL sel, void *replacement, void *original) {
    (void)ZeusInstallMessageHook(cls, sel, replacement, original, YES);
}
...
static void NullHookMessageIfPresent(Class cls, SEL sel, void *replacement, void *original) {
    (void)ZeusInstallMessageHook(cls, sel, replacement, original, NO);
}
...
static Class ZeusFirstClass(NSArray<NSString *> *names) {
...
static BOOL ZeusHookFirst(NSArray<NSString *> *classNames, NSArray<NSString *> *selectorNames, void *replacement, void *original) {
    Class cls = ZeusFirstClass(classNames);
    if (!cls) return NO;
