...
static NSString *ZeusDashExtractBaseURLAfterRangeInBlock(NSString *block, NSRange repTagRange);

@implementation ZeusDashVideoQuality
- (BOOL)isAV1 {
...
- (NSString *)menuSubtitle {
    ...
    if (self.estimatedOutputBytes > 0) {
        [parts addObject:[NSString stringWithFormat:@"~%@", ZeusDashFormatByteCount(self.estimatedOutputBytes)]];
    }
...
NSString *ZeusDashFormatByteCount(unsigned long long bytes) {
    if (bytes < 1024ULL) {
...
static NSString *ZeusDashXMLAttr(NSString *tag, NSString *name) {
...
static NSTimeInterval ZeusDashParseISODuration(NSString *value) {
...
NSTimeInterval ZeusDashManifestDuration(NSString *manifest) {
    ...
    for (NSString *pattern in patterns) {
        ...
        NSTimeInterval duration = ZeusDashParseISODuration([manifest substringWithRange:[match rangeAtIndex:1]]);
        if (duration > 0.05) return duration;
    }
    return 0;
}

static double ZeusDashParseFrameRate(NSString *value) {
    if (!value.length) return 0;
...
static NSString *ZeusDashCodecDisplayName(NSString *codecs) {
...
static NSInteger ZeusDashCodecSortRank(NSString *codecName) {
...
static unsigned long long ZeusDashEstimateConvertedBytes(NSInteger width, NSInteger height, NSInteger quality, double frameRate, NSTimeInterval duration, NSUInteger audioBandwidth) {
...
static NSUInteger ZeusDashBestAudioBandwidth(NSString *manifest) {
    if (!manifest.length) return 0;
...
NSArray<ZeusDashVideoQuality *> *ZeusDashManifestVideoQualities(NSString *manifest, NSTimeInterval fallbackDuration) {
    if (!manifest.length) return @[];

    NSTimeInterval duration = ZeusDashManifestDuration(manifest);
    if (duration <= 0.05) duration = fallbackDuration;
    NSUInteger audioBandwidth = ZeusDashBestAudioBandwidth(manifest);
    ...
    NSArray<NSTextCheckingResult *> *setMatches = [setRe matchesInString:manifest options:0 range:NSMakeRange(0, manifest.length)];
    NSMutableArray<ZeusDashVideoQuality *> *found = [NSMutableArray array];

    for (NSTextCheckingResult *setMatch in setMatches) {
        NSString *setTag = [manifest substringWithRange:setMatch.range];
        NSString *contentType = [ZeusDashXMLAttr(setTag, @"contentType") lowercaseString];
        ...
        for (NSTextCheckingResult *rm in repMatches) {
            ...
            NSString *url = ZeusDashExtractBaseURLAfterRangeInBlock(block, rm.range);
            url = [url stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
            if (!url.length) continue;

            NSString *codecs = ZeusDashXMLAttr(tag, @"codecs") ?: @"";
            NSString *codecName = ZeusDashCodecDisplayName(codecs);
            if ([codecName isEqualToString:@"Video"] && ![contentType isEqualToString:@"video"]) continue;

            NSInteger width = [ZeusDashXMLAttr(tag, @"width") integerValue];
            NSInteger height = [ZeusDashXMLAttr(tag, @"height") integerValue];
            NSString *label = ZeusDashXMLAttr(tag, @"FBQualityLabel");
            NSInteger quality = 0;
            if (label.length) {
            ...
            NSString *bwString = ZeusDashXMLAttr(tag, @"bandwidth");
            if (!bwString.length) bwString = ZeusDashXMLAttr(tag, @"FBBandwidth");
            NSUInteger bandwidth = (NSUInteger)[bwString longLongValue];
            NSString *fpsString = ZeusDashXMLAttr(tag, @"frameRate");
            if (!fpsString.length) fpsString = ZeusDashXMLAttr(tag, @"FBAvgFramerate");

            ZeusDashVideoQuality *item = [ZeusDashVideoQuality new];
            item.url = url;
            item.label = label;
            item.codecs = codecs;
            ...
            item.frameRate = ZeusDashParseFrameRate(fpsString);
            item.duration = duration;
            item.estimatedOutputBytes = ZeusDashEstimateConvertedBytes(width, height, quality, item.frameRate, duration, audioBandwidth);
            [found addObject:item];
        }
    }

    NSMutableDictionary<NSString *, ZeusDashVideoQuality *> *bestByKey = [NSMutableDictionary dictionary];
    NSMutableArray<ZeusDashVideoQuality *> *unkeyed = [NSMutableArray array];
    for (ZeusDashVideoQuality *item in found) {
        if (item.quality <= 0) {
            [unkeyed addObject:item];
            continue;
        }
        NSString *key = [NSString stringWithFormat:@"%ld-%@", (long)item.quality, item.codecName ?: @""];
        ZeusDashVideoQuality *existing = bestByKey[key];
        if (!existing || item.bandwidth > existing.bandwidth) {
            bestByKey[key] = item;
        }
    }

    NSMutableArray<ZeusDashVideoQuality *> *qualities = [[bestByKey allValues] mutableCopy];
    [qualities addObjectsFromArray:unkeyed];
    [qualities sortUsingComparator:^NSComparisonResult(ZeusDashVideoQuality *a, ZeusDashVideoQuality *b) {
        if (a.quality != b.quality) {
            return a.quality > b.quality ? NSOrderedAscending : NSOrderedDescending;
        }
        NSInteger rankA = ZeusDashCodecSortRank(a.codecName);
        NSInteger rankB = ZeusDashCodecSortRank(b.codecName);
        if (rankA != rankB) {
...
static NSInteger ZeusDashAudioCompatibilityScore(NSString *codecs) {
...
NSString *IGDashManifestBestAudioURL(NSString *manifest) {
    ...
    for (NSTextCheckingResult *setMatch in setMatches) {
        NSString *setTag = [manifest substringWithRange:setMatch.range];
        NSString *contentType = [ZeusDashXMLAttr(setTag, @"contentType") lowercaseString];
        NSString *mimeType = [ZeusDashXMLAttr(setTag, @"mimeType") lowercaseString];
        ...
        for (NSTextCheckingResult *rm in repMatches) {
            ...
            url = [url stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
            if (!url.length) continue;
            NSUInteger bw = (NSUInteger)[ZeusDashXMLAttr(tag, @"bandwidth") longLongValue];
            NSInteger score = ZeusDashAudioCompatibilityScore(ZeusDashXMLAttr(tag, @"codecs"));
    ...
    NSString *u = ZeusDashExtractBaseURLAfterRangeInBlock(block, lastMatch.range);
    return u.length ? u : nil;
}

static BOOL ZeusAVAssetLoadKeys(AVAsset *asset) {
    if (!asset) return NO;
...
static NSString *ZeusRenameAudioByMagic(NSString *path) {
    ...
    NSLog(@"ZeusPrepareDashAudioForMerge: rename failed %@", err);
    return path;
}

static NSString *ZeusTranscodeAudioToM4A(AVAsset *asset, NSString *sourcePath) {
    ...
    [session exportAsynchronouslyWithCompletionHandler:^{
        ...
        if (!ok) NSLog(@"ZeusPrepareDashAudioForMerge: AAC export failed %@ status %ld", session.error, (long)session.status);
        dispatch_semaphore_signal(sem);
    }];
...
NSString *ZeusPrepareDashAudioForMerge(NSString *audioPath) {
    ...
    NSString *path = ZeusRenameAudioByMagic(audioPath);
    ...
    ZeusAVAssetLoadKeys(asset);

    NSString *transcoded = ZeusTranscodeAudioToM4A(asset, path);
    if (transcoded.length) {
        if (![transcoded isEqualToString:path]) [fm removeItemAtPath:path error:nil];
        NSLog(@"ZeusPrepareDashAudioForMerge: transcoded DASH audio to AAC");
        return transcoded;
    }

    AVAssetTrack *track = [[asset tracksWithMediaType:AVMediaTypeAudio] firstObject];
    if (track) {
        NSLog(@"ZeusPrepareDashAudioForMerge: using original audio (AAC export unavailable)");
        return path;
    }
    NSLog(@"ZeusPrepareDashAudioForMerge: no decodable audio track in %@", path);
    return nil;
}

BOOL ZeusExportPhotosCompatibleMP4(NSString *videoPath, NSString *audioPath, BOOL hasAudio, NSString *outputPath) {
    ...
    ZeusAVAssetLoadKeys(videoAsset);
    ...
    if (![compVideo insertTimeRange:CMTimeRangeMake(kCMTimeZero, videoDur) ofTrack:videoTrack atTime:kCMTimeZero error:&insertErr]) {
        NSLog(@"ZeusExportPhotosCompatibleMP4: video insert failed %@", insertErr);
        return NO;
    }
    NSString *preparedAudio = nil;
    if (hasAudio && audioPath.length && [fm fileExistsAtPath:audioPath]) {
        preparedAudio = ZeusPrepareDashAudioForMerge(audioPath);
    }
    if (preparedAudio.length) {
        ...
        ZeusAVAssetLoadKeys(audioAsset);
        ...
        if (audioTrack) {
            ...
            if (![compAudio insertTimeRange:CMTimeRangeMake(kCMTimeZero, mergeDur) ofTrack:audioTrack atTime:kCMTimeZero error:&audioErr] || audioErr) {
                NSLog(@"ZeusExportPhotosCompatibleMP4: audio insert failed %@", audioErr);
            }
        } else {
            NSLog(@"ZeusExportPhotosCompatibleMP4: prepared audio has no track");
        }
    }
    ...
    for (NSString *preset in toTry) {
        ...
        if (!session) continue;
        // AVAssetExportSession RAISES NSInvalidArgumentException from
        // -setOutputFileType: when the type is not in supportedFileTypes. An AV1
        // composition has no MPEG-4 writer, so the list comes back without
        // AVFileTypeMPEG4 and the assignment below used to throw -- uncaught, on a
        // background queue, aborting the whole app with SIGABRT. Reached whenever
        // an AV1 rendition was picked and ffmpeg transcoding was unavailable.
        if (![session.supportedFileTypes containsObject:AVFileTypeMPEG4]) {
            NSLog(@"ZeusExportPhotosCompatibleMP4: preset %@ cannot write MPEG-4 (supports %@)",
                  preset, session.supportedFileTypes);
            continue;
        }
        ...
        @try {
            ...
            NSLog(@"ZeusExportPhotosCompatibleMP4: preset %@ rejected MPEG-4: %@", preset, exception);
            continue;
        }
        ...
        [session exportAsynchronouslyWithCompletionHandler:^{
            ok = (session.status == AVAssetExportSessionStatusCompleted);
            if (!ok) NSLog(@"ZeusExportPhotosCompatibleMP4 preset %@ failed: %@ status %ld", preset, session.error, (long)session.status);
            dispatch_semaphore_signal(sem);
        }];
        dispatch_semaphore_wait(sem, DISPATCH_TIME_FOREVER);
...
void ZeusPhotoLibraryImportVideoFromURL(NSURL *fileURL, void (^completion)(BOOL success, NSError *error)) {
    if (!fileURL) {
        if (completion) completion(NO, [NSError errorWithDomain:@"ZeusDashManifest" code:-2 userInfo:@{NSLocalizedDescriptionKey: @"Missing file URL"}]);
        return;
    }
