...
@implementation ZUProfileAnalyzerUser
...
+ (instancetype)userFromAPIDictionary:(NSDictionary *)d {
    ...
    ZUProfileAnalyzerUser *u = [self new];
    u.pk = pk;
...
- (instancetype)initWithDictionary:(NSDictionary *)dict {
    self = [super init];
    if (!self) return nil;
    ZUProfileAnalyzerUser *parsed = [ZUProfileAnalyzerUser userFromAPIDictionary:dict];
    if (parsed) {
        _pk = parsed.pk;
...
- (BOOL)isEqual:(id)object {
    ...
    if (![object isKindOfClass:[ZUProfileAnalyzerUser class]]) return NO;
    return [((ZUProfileAnalyzerUser *)object).pk isEqualToString:_pk];
}
...
@implementation ZUProfileAnalyzerSnapshot
...
@implementation ZUProfileAnalyzerDiffResult
@end

@implementation ZUProfileAnalyzerResult
@end
