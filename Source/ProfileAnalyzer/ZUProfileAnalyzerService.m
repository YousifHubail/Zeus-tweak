...
const NSInteger ZUProfileAnalyzerMaxAnalyzableFollowers = 13000;
...
@interface ZUProfileAnalyzerService () {
    NSInteger _expectedFollowers;
    NSInteger _expectedFollowing;
}
@property (nonatomic, strong, nullable) ZUProfileAnalyzerAPIClient *client;
@property (nonatomic, copy) NSString *selfPK;
@property (nonatomic, assign) BOOL cancelled;
@property (nonatomic, assign) BOOL running;
@end

static BOOL ZUPAErrorIndicatesIGThrottle(NSError *error) {
    if (!error) return NO;
...
static BOOL ZUPAShouldAvoidRetryHTTPCode(NSInteger code) {
    /* Don’t backoff-spam obviously permanent client errors. */
    return code == 401 || code == 402 || code == 404 || code == 405;
}

static double ZUPABackoffSecondsForAttempt(NSInteger retryAttemptZeroBased) {
...
- (NSError *)errorWithCode:(ZUProfileAnalyzerServiceError)code message:(NSString *)msg {
    return [NSError errorWithDomain:@"ZUProfileAnalyzerService" code:code
                           userInfo:@{ NSLocalizedDescriptionKey: msg ?: @"" }];
}

- (void)finishSnapshot:(nullable ZUProfileAnalyzerSnapshot *)s error:(nullable NSError *)e completion:(void (^)(ZUProfileAnalyzerSnapshot * _Nullable, NSError * _Nullable))completion {
    self.running = NO;
    self.cancelled = NO;
    if (completion) dispatch_async(dispatch_get_main_queue(), ^{ completion(s, e); });
}

- (void)reportProgress:(nullable ZUProfileAnalyzerFractionProgressBlock)p status:(NSString *)s fraction:(double)f {
    if (!p) return;
    dispatch_async(dispatch_get_main_queue(), ^{ p(s, f); });
}

- (void)runForSelfWithHeaderInfo:(ZUProfileAnalyzerHeaderInfoBlock)headerInfo
                        progress:(ZUProfileAnalyzerFractionProgressBlock)progress
                      completion:(void (^)(ZUProfileAnalyzerSnapshot * _Nullable, NSError * _Nullable))completion {
    if (self.running) {
        if (completion)
            dispatch_async(dispatch_get_main_queue(), ^{
                completion(nil, [self errorWithCode:ZUProfileAnalyzerServiceErrorCancelled message:@"Another analysis is already running."]);
            });
        return;
    }
    self.running = YES;
    ...
    if (!pk.length || !self.client) {
        [self finishSnapshot:nil error:[self errorWithCode:ZUProfileAnalyzerServiceErrorNoSession message:@"No active Instagram session or client."] completion:completion];
        return;
    }
...
- (void)performProfileInfoGETForPK:(NSString *)pk
                     httpRetryCount:(NSInteger)httpRetryCount
                         headerInfo:(ZUProfileAnalyzerHeaderInfoBlock)headerInfo
                           progress:(ZUProfileAnalyzerFractionProgressBlock)progress
                         completion:(void (^)(ZUProfileAnalyzerSnapshot * _Nullable, NSError * _Nullable))completion {
    ZUProfileAnalyzerAPIClient *client = self.client;
    ...
    [client GETWithEndpointPath:[NSString stringWithFormat:@"users/%@/info/", pk] queryParams:nil success:^(NSDictionary * _Nullable resp) {
        ...
        if (strongSelf.cancelled) {
            [strongSelf finishSnapshot:nil error:[strongSelf errorWithCode:ZUProfileAnalyzerServiceErrorCancelled message:@"Cancelled"] completion:completion];
            return;
        }
        ...
        if (!user) {
            [strongSelf finishSnapshot:nil error:[strongSelf errorWithCode:ZUProfileAnalyzerServiceErrorNetwork message:@"Couldn't fetch profile information"] completion:completion];
            return;
        }
        ...
        if (followerCount > ZUProfileAnalyzerMaxAnalyzableFollowers) {
            [strongSelf finishSnapshot:nil error:[strongSelf errorWithCode:ZUProfileAnalyzerServiceErrorTooManyFollowers message:@"Too many followers to analyze"] completion:completion];
            return;
        }

        ZUProfileAnalyzerSnapshot *snap = [[ZUProfileAnalyzerSnapshot alloc] initWithUserPK:pk scannedAt:[NSDate date] followers:@[] following:@[]];
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(TH_PA_AFTER_PROFILE_INFO_DELAY_S * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if (!s2) return;
            if (s2.cancelled) {
                [s2 finishSnapshot:nil error:[s2 errorWithCode:ZUProfileAnalyzerServiceErrorCancelled message:@"Cancelled"] completion:completion];
                return;
            }
        ...
        NSInteger code = error.code;
        if (httpRetryCount < TH_PA_MAX_PAGE_FAILURE_RETRIES && ZUPAErrorIndicatesIGThrottle(error) && !ZUPAShouldAvoidRetryHTTPCode(code)) {
            double wait = ZUPABackoffSecondsForAttempt(httpRetryCount);
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(wait * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                if (!s3 || s3.cancelled) {
                    if (s3) [s3 finishSnapshot:nil error:[s3 errorWithCode:ZUProfileAnalyzerServiceErrorCancelled message:@"Cancelled"] completion:completion];
                    return;
                }
        ...
        NSError *wrapped = [strongSelf errorWithCode:ZUProfileAnalyzerServiceErrorNetwork message:error.localizedDescription ?: @"Network error"];
        if (ZUPAErrorIndicatesIGThrottle(error))
            wrapped = [strongSelf errorWithCode:ZUProfileAnalyzerServiceErrorRateLimited message:@"Instagram is limiting these requests. Wait a while, then try again."];
...
- (void)fetchFollowersForPK:(NSString *)pk
                   snapshot:(ZUProfileAnalyzerSnapshot *)snap
                   progress:(ZUProfileAnalyzerFractionProgressBlock)progress
                 completion:(void (^)(ZUProfileAnalyzerSnapshot * _Nullable, NSError * _Nullable))completion {
    NSMutableArray<ZUProfileAnalyzerUser *> *acc = [NSMutableArray array];
 ...
 pageHTTPRetryCount:0
          progress:progress
        completion:^(NSArray<ZUProfileAnalyzerUser *> *users, NSError * _Nullable error) {
        if (error) {
...
static NSString *_Nullable ZUPANextCursor(id next) {
    if (!next) return nil;
...
- (void)pagePath:(NSString *)basePath
             acc:(NSMutableArray<ZUProfileAnalyzerUser *> *)acc
           maxID:(NSString *)maxID
           total:(NSInteger)total
           stage:(NSString *)stage
 pageHTTPRetryCount:(NSInteger)pageHTTPRetryCount
        progress:(ZUProfileAnalyzerFractionProgressBlock)progress
      completion:(void (^)(NSArray<ZUProfileAnalyzerUser *> *users, NSError * _Nullable error))completion {
    if (self.cancelled) {
        completion(nil, [self errorWithCode:ZUProfileAnalyzerServiceErrorCancelled message:@"Cancelled"]);
        return;
    }
    ...
    [client GETWithEndpointPath:basePath queryParams:query success:^(NSDictionary * _Nullable resp) {
        typeof(self) strongSelf = weakSelf;
        if (!strongSelf) {
            completion(nil, [NSError errorWithDomain:@"ZUProfileAnalyzerService" code:-1 userInfo:@{ NSLocalizedDescriptionKey: @"Service released." }]);
            return;
        }
        if (!resp) {
            completion(nil, strongSelf.cancelled ? [strongSelf errorWithCode:ZUProfileAnalyzerServiceErrorCancelled message:@"Cancelled"] : [strongSelf errorWithCode:ZUProfileAnalyzerServiceErrorNetwork message:@"Empty response"]);
            return;
        }
        ...
        if ([users isKindOfClass:[NSArray class]]) {
            for (NSDictionary *d in users) {
                if ([d isKindOfClass:[NSDictionary class]]) {
                    ZUProfileAnalyzerUser *u = [ZUProfileAnalyzerUser userFromAPIDictionary:d];
        ...
        NSString *nextMax = ZUPANextCursor(resp[@"next_max_id"]);
        if (!nextMax.length || strongSelf.cancelled) {
            completion(acc, strongSelf.cancelled ? [strongSelf errorWithCode:ZUProfileAnalyzerServiceErrorCancelled message:@"Cancelled"] : nil);
            return;
        }

        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(ZUPAPageDelayWithJitter() * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (strongSelf.cancelled) {
            completion(nil, [strongSelf errorWithCode:ZUProfileAnalyzerServiceErrorCancelled message:@"Cancelled"]);
            return;
        }
        NSInteger code = error.code;
        if (pageHTTPRetryCount < TH_PA_MAX_PAGE_FAILURE_RETRIES && ZUPAErrorIndicatesIGThrottle(error) && !ZUPAShouldAvoidRetryHTTPCode(code)) {
            double wait = ZUPABackoffSecondsForAttempt(pageHTTPRetryCount);
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(wait * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                if (s2.cancelled) {
                    completion(nil, [s2 errorWithCode:ZUProfileAnalyzerServiceErrorCancelled message:@"Cancelled"]);
                    return;
                }
        ...
        if (ZUPAErrorIndicatesIGThrottle(error))
            outErr = [strongSelf errorWithCode:ZUProfileAnalyzerServiceErrorRateLimited message:@"Instagram is limiting these requests. Wait a while, then try again."];
