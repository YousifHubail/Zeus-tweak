#import "Include/ZUProfileAnalyzerViewController.h"
...
#import "Include/ZUProfileAnalyzerService.h"
#import "Include/ZUProfileAnalyzerDiffEngine.h"
#import "Include/ZUProfileAnalyzerStorage.h"
...
static const NSInteger kZUProfileAnalyzerMaxTotal = 13000;

static NSString * const kZUProfileAnalyzerPinnedPKsKey = @"ZUProfileAnalyzerPinnedPKs";
static NSString * const kZUProfileAnalyzerIgnoredPKsKey = @"ZUProfileAnalyzerIgnoredPKs";
static NSString * const kZUProfileAnalyzerAPICountsPrefix = @"ZUProfileAnalyzerAPICounts_";

static NSMutableSet<NSString *> *ZUProfileAnalyzerMutableSetForKey(NSString *key) {
...
static void ZUProfileAnalyzerSaveSetForKey(NSSet<NSString *> *set, NSString *key) {
    NSUserDefaults *d = [NSUserDefaults standardUserDefaults];
...
static id _Nullable ZUGetIvar(id obj, const char *ivarName) {
...
static id _Nullable ZUProfileAnalyzerGetNetworker(void) {
    id networker = nil;
    @try {
        id delegate = [[UIApplication sharedApplication] delegate];
        if (!delegate) return nil;
        id appCoordinator = ZUGetIvar(delegate, "_appCoordinator");
        if (!appCoordinator) appCoordinator = [delegate valueForKey:@"_appCoordinator"];
        if (!appCoordinator) appCoordinator = ZUGetIvar([delegate valueForKey:@"_delegate"], "_appCoordinator");
        if (appCoordinator) {
            id sessionManager = ZUGetIvar(appCoordinator, "_sessionManager");
            if (!sessionManager) sessionManager = [appCoordinator valueForKey:@"_sessionManager"];
            if (sessionManager) {
                id activeUserSessions = ZUGetIvar(sessionManager, "_activeUserSessions");
                ...
                if (activeUserSessions) {
                    ...
                    if (session) {
                        ...
                        if (!userActions) userActions = ZUGetIvar(session, "_userActions");
                        if (userActions) {
                            networker = [userActions valueForKey:@"networker"];
                            if (!networker) networker = ZUGetIvar(userActions, "_networker");
                        }
                    }
                }
            }
        ...
        if (!networker) {
            ...
            if ([windows isKindOfClass:[NSArray class]]) {
                ...
                for (UIWindow *w in windows) {
                    ...
                    id userSession = ZUGetIvar(w, "_userSession");
                    ...
                    if (!userActions) userActions = ZUGetIvar(userSession, "_userActions");
...
static NSURLRequest * _Nullable ZUProfileAnalyzerBuildIGRequest(NSString *path, NSDictionary<NSString *, NSString *> * _Nullable queryParams, NSInteger method) {
...
static NSURLRequest * _Nullable ZUProfileAnalyzerBuildIGURLRequest(NSURL *url, NSInteger method) {
...
static void ZUProfileAnalyzerParseAPIURL(NSURL *url, NSString * _Nullable *outPath, NSDictionary<NSString *, NSString *> * _Nullable *outParams) {
...
static void ZUProfileAnalyzerOpenProfileForUsername(NSString *username) {
...
@interface ZUProfileAnalyzerUserHistoryViewController : UIViewController <UITableViewDataSource, UITableViewDelegate>
- (instancetype)initWithUserPK:(NSString *)userPK username:(NSString * _Nullable)username currentUserPK:(NSString *)currentUserPK;
@end

@class ZUProfileAnalyzerScanHistoryViewController;

@interface ZUProfileAnalyzerScanCompareGraphView : UIView
@property (nonatomic, copy) NSArray<ZUProfileAnalyzerSnapshot *> *scans;
@property (nonatomic, assign) BOOL isDarkMode;
@end

@interface ZUProfileAnalyzerScanCompareViewController : UIViewController
- (instancetype)initWithScans:(NSArray<ZUProfileAnalyzerSnapshot *> *)scans;
@end

@interface ZUProfileAnalyzerListViewController : UIViewController <UITableViewDataSource, UITableViewDelegate, UISearchResultsUpdating>
- (instancetype)initWithTitle:(NSString *)title users:(NSArray<ZUProfileAnalyzerUser *> *)users;
@property (nonatomic, copy) NSString *currentUserPK;
@end

@implementation ZUProfileAnalyzerListViewController {
    NSString *_listTitle;
    NSString *_currentUserPK;
    NSArray<ZUProfileAnalyzerUser *> *_allUsers;
    NSArray<ZUProfileAnalyzerUser *> *_users;
...
- (instancetype)initWithTitle:(NSString *)title users:(NSArray<ZUProfileAnalyzerUser *> *)users {
    if (self = [super init]) {
        ...
        NSArray<ZUProfileAnalyzerUser *> *source = [users copy] ?: @[];
        // Sort users by username (fallback to PK)
        _allUsers = [source sortedArrayUsingComparator:^NSComparisonResult(ZUProfileAnalyzerUser *a, ZUProfileAnalyzerUser *b) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ZUProfileAnalyzerUser *u = _users[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (username.length) ZUProfileAnalyzerOpenProfileForUsername(username);
}
...
- (void)tableView:(UITableView *)tableView didLongPressRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *pk = u.pk;
    NSMutableSet<NSString *> *pinned = ZUProfileAnalyzerMutableSetForKey(kZUProfileAnalyzerPinnedPKsKey);
    NSMutableSet<NSString *> *ignored = ZUProfileAnalyzerMutableSetForKey(kZUProfileAnalyzerIgnoredPKsKey);
    ...
    [ac addAction:[UIAlertAction actionWithTitle:pinTitle style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *action) {
        ...
        ZUProfileAnalyzerSaveSetForKey(pinned, kZUProfileAnalyzerPinnedPKsKey);
    }]];
    ...
    [ac addAction:[UIAlertAction actionWithTitle:ignoreTitle style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *action) {
        ...
        ZUProfileAnalyzerSaveSetForKey(ignored, kZUProfileAnalyzerIgnoredPKsKey);
    }]];
    if (_currentUserPK.length) {
        [ac addAction:[UIAlertAction actionWithTitle:@"View history" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *action) {
            ZUProfileAnalyzerUserHistoryViewController *historyVC = [[ZUProfileAnalyzerUserHistoryViewController alloc] initWithUserPK:pk username:u.username.length ? u.username : nil currentUserPK:_currentUserPK];
...
- (void)updateSearchResultsForSearchController:(UISearchController *)searchController {
    ...
    NSMutableArray<ZUProfileAnalyzerUser *> *filtered = [NSMutableArray array];
    for (ZUProfileAnalyzerUser *u in _allUsers) {
...
static NSInteger ZUProfileAnalyzerEffectiveFollowersCount(ZUProfileAnalyzerSnapshot *s) {
    return s.apiFollowersCount >= 0 ? s.apiFollowersCount : (NSInteger)s.followers.count;
}
static NSInteger ZUProfileAnalyzerEffectiveFollowingCount(ZUProfileAnalyzerSnapshot *s) {
    return s.apiFollowingCount >= 0 ? s.apiFollowingCount : (NSInteger)s.following.count;
}

// MARK: - Scan History
@interface ZUProfileAnalyzerScanHistoryViewController : UIViewController <UITableViewDataSource, UITableViewDelegate>
...
- (instancetype)initWithCurrentUserPK:(NSString *)userPK currentSnapshot:(ZUProfileAnalyzerSnapshot * _Nullable)currentSnapshot;
@end

@implementation ZUProfileAnalyzerScanHistoryViewController {
    NSString *_currentUserPK;
    ZUProfileAnalyzerSnapshot *_currentSnapshot;
    NSArray<ZUProfileAnalyzerSnapshot *> *_scans; /** Table order: oldest at row 0, newest at bottom. Compare to loadSnapshotAtIndex:0 == newest (newest-first index = count-1-row). */
    UITableView *_tableView;
    UILabel *_emptyLabel;
    BOOL _selectionMode;
...
- (void)scanHistory_reloadFromStorageNotifyParent:(BOOL)notify {
    ZUProfileAnalyzerStorage *storage = [[ZUProfileAnalyzerStorage alloc] init];
    NSMutableArray<ZUProfileAnalyzerSnapshot *> *scans = [NSMutableArray array];
    for (NSInteger idx = 0; ; idx++) {
        ZUProfileAnalyzerSnapshot *snap = [storage loadSnapshotAtIndex:idx forUserPK:_currentUserPK error:NULL];
        if (!snap) break;
        [scans addObject:snap];
    }
    _scans = [[[scans reverseObjectEnumerator] allObjects] copy]; // table: oldest first (row 0)
    ZUProfileAnalyzerSnapshot *latest = [storage loadSnapshotAtIndex:0 forUserPK:_currentUserPK error:NULL];
...
- (void)scanHistory_compareSelected {
    ...
    NSMutableArray<ZUProfileAnalyzerSnapshot *> *selected = [NSMutableArray array];
    ...
    [selected sortUsingComparator:^NSComparisonResult(ZUProfileAnalyzerSnapshot *a, ZUProfileAnalyzerSnapshot *b) {
        return [a.scannedAt compare:b.scannedAt];
    }];
    ZUProfileAnalyzerScanCompareViewController *compareVC = [[ZUProfileAnalyzerScanCompareViewController alloc] initWithScans:[selected copy]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ZUProfileAnalyzerSnapshot *snap = _scans[indexPath.row];
    NSDateFormatter *fmt = [[NSDateFormatter alloc] init];
    fmt.dateStyle = NSDateFormatterMediumStyle;
    fmt.timeStyle = NSDateFormatterShortStyle;
    cell.textLabel.text = [fmt stringFromDate:snap.scannedAt];
    NSInteger f = ZUProfileAnalyzerEffectiveFollowersCount(snap), g = ZUProfileAnalyzerEffectiveFollowingCount(snap);
    ...
    if (_currentSnapshot && snap != _currentSnapshot) {
        NSInteger curF = ZUProfileAnalyzerEffectiveFollowersCount(_currentSnapshot), curG = ZUProfileAnalyzerEffectiveFollowingCount(_currentSnapshot);
...
@implementation ZUProfileAnalyzerScanCompareGraphView
- (void)setScans:(NSArray<ZUProfileAnalyzerSnapshot *> *)scans {
    _scans = [scans copy];
...
- (void)drawRect:(CGRect)rect {
    ...
    NSInteger maxVal = 0;
    for (ZUProfileAnalyzerSnapshot *s in _scans) {
    ...
    for (NSInteger i = 0; i < n; i++) {
        ZUProfileAnalyzerSnapshot *s = _scans[i];
...
@implementation ZUProfileAnalyzerScanCompareViewController {
    NSArray<ZUProfileAnalyzerSnapshot *> *_scans;
    ZUProfileAnalyzerScanCompareGraphView *_graphView;
...
- (void)viewDidLoad {
    ...
    _graphView = [[ZUProfileAnalyzerScanCompareGraphView alloc] initWithFrame:CGRectMake(0, 0, w, graphHeight)];
...
typedef NS_ENUM(NSInteger, ZUProfileAnalyzerUserHistoryRowKind) {
    ZUProfileAnalyzerUserHistoryRowSummaryFirst,
    ZUProfileAnalyzerUserHistoryRowSummaryLast,
    ZUProfileAnalyzerUserHistoryRowSummaryCount,
    ZUProfileAnalyzerUserHistoryRowScan
};

@implementation ZUProfileAnalyzerUserHistoryViewController {
    NSString *_userPK;
    NSString *_username;
    NSString *_currentUserPK;
    NSArray<ZUProfileAnalyzerSnapshot *> *_scans; // newest first
...
static BOOL _userPKInArray(NSString *pk, NSArray<ZUProfileAnalyzerUser *> *arr) {
    if (!pk.length) return NO;
    for (ZUProfileAnalyzerUser *u in arr) if ([u.pk isEqualToString:pk]) return YES;
    return NO;
}
...
- (void)viewDidLoad {
    ...
    NSInteger count = 0;
    NSArray<ZUProfileAnalyzerSnapshot *> *scansNewestFirst = _scans;
    for (NSInteger i = 0; i < (NSInteger)scansNewestFirst.count; i++) {
        ZUProfileAnalyzerSnapshot *snap = scansNewestFirst[i];
        ...
        if (i + 1 < (NSInteger)scansNewestFirst.count) {
            ZUProfileAnalyzerSnapshot *older = scansNewestFirst[i + 1];
...
@interface ZUProfileAnalyzerNetworkDelegate : NSObject <ZUProfileAnalyzerAPIClientNetworkDelegate>
@end

@implementation ZUProfileAnalyzerNetworkDelegate
- (void)performGETRequestWithURL:(NSURL *)url success:(void (^)(NSDictionary * _Nullable))success failure:(void (^)(NSError * _Nonnull))failure {
    id networker = ZUProfileAnalyzerGetNetworker();
    NSString *endpointPath = nil;
    NSDictionary *queryParams = nil;
    ZUProfileAnalyzerParseAPIURL(url, &endpointPath, &queryParams);
    ...
    void (^onFailure)(id) = ^(id err) {
        ...
        if ([err isKindOfClass:[NSError class]]) {
            ...
            e = [NSError errorWithDomain:@"ZUProfileAnalyzer" code:code userInfo:@{ NSLocalizedDescriptionKey: msg }];
        } else {
            e = [NSError errorWithDomain:@"ZUProfileAnalyzer" code:401 userInfo:@{ NSLocalizedDescriptionKey: err ? [err description] : @"Request failed" }];
        }
    ...
    if (networker) {
        ...
        if (endpointPath.length)
            igRequest = ZUProfileAnalyzerBuildIGRequest(endpointPath, queryParams, 0);
        if (!igRequest)
            igRequest = ZUProfileAnalyzerBuildIGURLRequest(url, 0);
        if (igRequest) {
    ...
    [[session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        if (code != 200 && code != 0) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{ failure([NSError errorWithDomain:@"ZUProfileAnalyzer" code:code userInfo:@{ NSLocalizedDescriptionKey: msg }]); });
            return;
        }
...
typedef NS_ENUM(NSInteger, ZUProfileAnalyzerMetric) {
    ZUProfileAnalyzerMetricNewFollowers,
    ZUProfileAnalyzerMetricLostFollowers,
    ZUProfileAnalyzerMetricYouFollowed,
    ZUProfileAnalyzerMetricYouUnfollowed,
    ZUProfileAnalyzerMetricNotFollowingYouBack,
    ZUProfileAnalyzerMetricYouDontFollowBack,
    ZUProfileAnalyzerMetricMutualFollowers,
    ZUProfileAnalyzerMetricCount
};

@interface ZUProfileAnalyzerViewController () <UITableViewDataSource, UITableViewDelegate>
...
@property (nonatomic, strong, nullable) ZUProfileAnalyzerResult *result;
@property (nonatomic, strong, nullable) ZUProfileAnalyzerDiffResult *previousDiff;
@property (nonatomic, copy) NSString *currentUserPK;
...
@property (nonatomic, strong, nullable) ZUProfileAnalyzerService *profileAnalyzerScanner;
@end

@implementation ZUProfileAnalyzerViewController
...
+ (NSString *)currentUserUsernameFromInstagram {
    @try {
        ...
        for (UIWindow *w in windows) {
            if (!w || ![w isKindOfClass:igWindowClass]) continue;
            id session = ZUGetIvar(w, "_userSession");
            if (!session) session = [w valueForKey:@"userSession"];
            if (!session) continue;
            id user = ZUGetIvar(session, "_user");
...
- (void)viewDidLoad {
    ...
    _currentUserPK = [ZUProfileAnalyzerViewController currentUserPKFromInstagram];
    if (!_currentUserPK.length) {
...
static NSArray<NSString *> *ZUProfileAnalyzerMetricTitles(void) {
    return @[
        @"New followers",
        @"Lost followers",
        @"You followed",
...
- (void)buildDashboardUI {
    ...
    _usernameLabel.text = [ZUProfileAnalyzerViewController currentUserUsernameFromInstagram] ?: @"—";
    ...
    [_infoButton setTitleColor:[ZeusHelper iotaPinkColor] forState:UIControlStateNormal];
    ...
    [_scanNowButton setTitleColor:[ZeusHelper iotaPinkColor] forState:UIControlStateNormal];
    ...
    _activityView.color = [ZeusHelper iotaPinkColor];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return ZUProfileAnalyzerMetricCount + 1; // metrics + scan history row
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (row >= ZUProfileAnalyzerMetricCount) {
        // Scan history row
    ...
    NSInteger tag = row;
    NSArray<NSString *> *titles = ZUProfileAnalyzerMetricTitles();
    cell.textLabel.text = tag < (NSInteger)titles.count ? titles[tag] : @"";
    NSInteger count = [self countForMetric:(ZUProfileAnalyzerMetric)tag];
    NSInteger prev = [self previousCountForMetric:(ZUProfileAnalyzerMetric)tag];
    ...
    if (showDelta) {
        ...
        cell.detailTextLabel.textColor = [ZeusHelper iotaPinkColor]; /* accent same in both themes */
    }
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSInteger tag = row;
    if (tag < 0 || tag >= ZUProfileAnalyzerMetricCount || !_result.diff) return;
    [self openMetricListForTag:tag];
}

static NSString *ZUProfileAnalyzerProfileImageCachePath(NSString *userPK) {
    if (!userPK.length) return nil;
    return [[[ZUProfileAnalyzerStorage profileAnalyzerDirectory] stringByAppendingPathComponent:userPK] stringByAppendingPathComponent:@"profile.jpg"];
}

/* Download image from URL and write to cache path. Call from any queue; no UI updates. */
static void ZUProfileAnalyzerDownloadAndCacheProfileImage(NSString *urlString, NSString *userPK) {
    if (!urlString.length || !userPK.length) return;
    NSURL *url = [NSURL URLWithString:urlString];
    if (!url) return;
    NSString *cachePath = ZUProfileAnalyzerProfileImageCachePath(userPK);
    if (!cachePath) return;
    NSURLSessionDataTask *task = [[NSURLSession sharedSession] dataTaskWithURL:url completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        (void)response;
        (void)error;
...
+ (void)prefetchProfileImageIfNeeded {
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
        ...
        if (!pk.length) return;
        NSString *cachePath = ZUProfileAnalyzerProfileImageCachePath(pk);
        ...
        if (urlString.length) {
            ZUProfileAnalyzerDownloadAndCacheProfileImage(urlString, pk);
            return;
        }

        /* Fallback: fetch users/{pk}/info/ and cache profile_pic_url. API must run on main (IG networker). */
        dispatch_async(dispatch_get_main_queue(), ^{
            ZUProfileAnalyzerAPIClient *client = [[ZUProfileAnalyzerAPIClient alloc] init];
            client.networkDelegate = [[ZUProfileAnalyzerNetworkDelegate alloc] init];
            ...
            [client GETWithEndpointPath:path queryParams:nil success:^(NSDictionary * _Nullable json) {
                ...
                if ([picURL isKindOfClass:[NSString class]] && ((NSString *)picURL).length)
                    dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
                        ZUProfileAnalyzerDownloadAndCacheProfileImage((NSString *)picURL, pk);
                    });
...
- (void)loadCurrentUserProfileImage {
    if (_currentUserPK.length) {
        NSString *cachePath = ZUProfileAnalyzerProfileImageCachePath(_currentUserPK);
        if (cachePath.length) {
    ...
    NSString *urlString = [ZUProfileAnalyzerViewController currentUserProfilePicURLFromInstagram];
    if (urlString.length) {
...
static void ZUProfileAnalyzerParseCountsFromUser(NSDictionary *user, NSInteger *outFollowers, NSInteger *outFollowing) {
    *outFollowers = -1;
    *outFollowing = -1;
    if (![user isKindOfClass:[NSDictionary class]]) return;
...
- (void)loadSavedAPICounts {
    if (!_currentUserPK.length) return;
    NSString *keyBase = [kZUProfileAnalyzerAPICountsPrefix stringByAppendingString:_currentUserPK];
...
- (void)infoTapped {
    ...
    [ZeusHelper showCustomAlertWithActions:@"Profile Analyzer" description:@"Scan your profile to see new followers, unfollowers, who you follow, who doesn't follow back, and more. Tap \"Scan now\" to run a scan, then tap a metric to see the list." actions:@[
        @{
            @"title": @"OK",
            @"handler": ^(id sender) {
            }
...
- (void)runScan {
    ...
    ZUProfileAnalyzerAPIClient *client = [[ZUProfileAnalyzerAPIClient alloc] init];
    self.networkDelegate = [[ZUProfileAnalyzerNetworkDelegate alloc] init];
    client.networkDelegate = self.networkDelegate;
    ZUProfileAnalyzerStorage *storage = [[ZUProfileAnalyzerStorage alloc] init];
    ZUProfileAnalyzerSnapshot *previous = [storage loadMostRecentSnapshotForUserPK:_currentUserPK error:NULL];
    ZUProfileAnalyzerService *scanner = [[ZUProfileAnalyzerService alloc] initWithAPIClient:client userPK:_currentUserPK];
    ...
    [scanner runForSelfWithHeaderInfo:^(NSDictionary *userInfo) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            NSInteger fc = -1, fg = -1;
            ZUProfileAnalyzerParseCountsFromUser(userInfo, &fc, &fg);
            if (fc >= 0 && fg >= 0) {
    ...
    } completion:^(ZUProfileAnalyzerSnapshot * _Nullable snapshot, NSError * _Nullable error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (error) {
                ...
                if (ENABLED(@"Show Banners")) {
                    [ZeusHelper showToastWithTitle:@"Profile Analyzer" subtitle:error.localizedDescription icon:[UIImage systemImageNamed:@"exclamationmark.triangle"] autoHide:4 openURL:nil];
                }
                return;
            }
            if (!snapshot) {
            ...
            if (kZUProfileAnalyzerMaxTotal > 0 && total > kZUProfileAnalyzerMaxTotal) {
                wself.statusLabel.text = [NSString stringWithFormat:@"Followers + following (%td) exceeds %td. Instagram limits.", total, (NSInteger)kZUProfileAnalyzerMaxTotal];
                return;
            }
            wself.statusLabel.text = @"Comparing…";
            NSMutableDictionary *usersByPK = [[ZUProfileAnalyzerDiffEngine usersByPKFromSnapshot:snapshot] mutableCopy];
            if (previous) {
                for (ZUProfileAnalyzerUser *u in previous.followers) if (u.pk.length && !usersByPK[u.pk]) usersByPK[u.pk] = u;
                for (ZUProfileAnalyzerUser *u in previous.following) if (u.pk.length && !usersByPK[u.pk]) usersByPK[u.pk] = u;
            }
            ZUProfileAnalyzerDiffResult *diff = [ZUProfileAnalyzerDiffEngine diffBetweenPreviousSnapshot:previous currentSnapshot:snapshot usersByPK:usersByPK];
            [storage saveSnapshot:snapshot error:NULL];
            ZUProfileAnalyzerResult *result = [[ZUProfileAnalyzerResult alloc] init];
            ...
            if (ENABLED(@"Show Banners")) {
                [ZeusHelper showToastWithTitle:@"Scan complete" subtitle:[NSString stringWithFormat:@"%td followers, %td following", result.followersCount, result.followingCount] icon:[UIImage systemImageNamed:@"checkmark.circle.fill"] autoHide:3 openURL:nil];
            }
        });
    }];
}
...
- (void)loadPreviousDiffFromStorage {
    ...
    ZUProfileAnalyzerStorage *storage = [[ZUProfileAnalyzerStorage alloc] init];
    ZUProfileAnalyzerSnapshot *prevPrev = [storage loadSnapshotAtIndex:2 forUserPK:_currentUserPK error:NULL];
    if (!prevPrev) return;
    NSMutableDictionary *usersByPK = [[ZUProfileAnalyzerDiffEngine usersByPKFromSnapshot:_result.previousSnapshot] mutableCopy];
    for (ZUProfileAnalyzerUser *u in prevPrev.followers) if (u.pk.length && !usersByPK[u.pk]) usersByPK[u.pk] = u;
    for (ZUProfileAnalyzerUser *u in prevPrev.following) if (u.pk.length && !usersByPK[u.pk]) usersByPK[u.pk] = u;
    self.previousDiff = [ZUProfileAnalyzerDiffEngine diffBetweenPreviousSnapshot:prevPrev currentSnapshot:_result.previousSnapshot usersByPK:usersByPK];
}

- (void)loadLastResultAndRefresh {
    if (!_currentUserPK.length) return;
    ZUProfileAnalyzerStorage *storage = [[ZUProfileAnalyzerStorage alloc] init];
    ZUProfileAnalyzerSnapshot *current = [storage loadSnapshotAtIndex:0 forUserPK:_currentUserPK error:NULL];
    ZUProfileAnalyzerSnapshot *previous = [storage loadSnapshotAtIndex:1 forUserPK:_currentUserPK error:NULL];
    if (!current) {
        self.scanCount = 0;
    ...
    NSMutableDictionary *usersByPK = [[ZUProfileAnalyzerDiffEngine usersByPKFromSnapshot:current] mutableCopy];
    if (previous) {
        for (ZUProfileAnalyzerUser *u in previous.followers) if (u.pk.length && !usersByPK[u.pk]) usersByPK[u.pk] = u;
        for (ZUProfileAnalyzerUser *u in previous.following) if (u.pk.length && !usersByPK[u.pk]) usersByPK[u.pk] = u;
    }
    ZUProfileAnalyzerDiffResult *diff = [ZUProfileAnalyzerDiffEngine diffBetweenPreviousSnapshot:previous currentSnapshot:current usersByPK:usersByPK];
    ZUProfileAnalyzerResult *result = [[ZUProfileAnalyzerResult alloc] init];
    ...
    if (previous) {
        ZUProfileAnalyzerSnapshot *prevPrev = [storage loadSnapshotAtIndex:2 forUserPK:_currentUserPK error:NULL];
        if (prevPrev) {
            NSMutableDictionary *prevUsers = [[ZUProfileAnalyzerDiffEngine usersByPKFromSnapshot:previous] mutableCopy];
            for (ZUProfileAnalyzerUser *u in prevPrev.followers) if (u.pk.length && !prevUsers[u.pk]) prevUsers[u.pk] = u;
            for (ZUProfileAnalyzerUser *u in prevPrev.following) if (u.pk.length && !prevUsers[u.pk]) prevUsers[u.pk] = u;
            self.previousDiff = [ZUProfileAnalyzerDiffEngine diffBetweenPreviousSnapshot:prevPrev currentSnapshot:previous usersByPK:prevUsers];
        }
    }
    [self refreshDashboard];
}

- (NSInteger)countForMetric:(ZUProfileAnalyzerMetric)metric {
    if (!_result.diff) return 0;
    ZUProfileAnalyzerDiffResult *d = _result.diff;
    switch (metric) {
        case ZUProfileAnalyzerMetricNewFollowers: return d.followersGained.count;
        case ZUProfileAnalyzerMetricLostFollowers: return d.followersLost.count;
        case ZUProfileAnalyzerMetricYouFollowed: return d.followingAdded.count;
        case ZUProfileAnalyzerMetricYouUnfollowed: return d.followingRemoved.count;
        case ZUProfileAnalyzerMetricNotFollowingYouBack: return d.notFollowingMeBack.count;
        case ZUProfileAnalyzerMetricYouDontFollowBack: return d.youDontFollowBack.count;
        case ZUProfileAnalyzerMetricMutualFollowers: return d.mutualFollowers.count;
        default: return 0;
    }
}

- (NSInteger)previousCountForMetric:(ZUProfileAnalyzerMetric)metric {
    if (!_previousDiff) return 0;
    ZUProfileAnalyzerDiffResult *d = _previousDiff;
    switch (metric) {
        case ZUProfileAnalyzerMetricNewFollowers: return d.followersGained.count;
        case ZUProfileAnalyzerMetricLostFollowers: return d.followersLost.count;
        case ZUProfileAnalyzerMetricYouFollowed: return d.followingAdded.count;
...
- (void)openScanHistory {
    if (!_currentUserPK.length) return;
    ZUProfileAnalyzerStorage *storage = [[ZUProfileAnalyzerStorage alloc] init];
    ZUProfileAnalyzerSnapshot *any = [storage loadSnapshotAtIndex:0 forUserPK:_currentUserPK error:NULL];
    if (!any) {
    ...
    ZUProfileAnalyzerScanHistoryViewController *vc = [[ZUProfileAnalyzerScanHistoryViewController alloc] initWithCurrentUserPK:_currentUserPK currentSnapshot:_result.currentSnapshot];
...
- (void)openMetricListForTag:(NSInteger)tag {
    if (tag < 0 || tag >= ZUProfileAnalyzerMetricCount || !_result.diff) return;
    NSArray<ZUProfileAnalyzerUser *> *users = nil;
    NSString *listTitle = nil;
    ZUProfileAnalyzerDiffResult *d = _result.diff;
    switch ((ZUProfileAnalyzerMetric)tag) {
        case ZUProfileAnalyzerMetricNewFollowers: users = d.followersGained; listTitle = @"New followers"; break;
        case ZUProfileAnalyzerMetricLostFollowers: users = d.followersLost; listTitle = @"Lost followers"; break;
        case ZUProfileAnalyzerMetricYouFollowed: users = d.followingAdded; listTitle = @"You followed"; break;
        case ZUProfileAnalyzerMetricYouUnfollowed: users = d.followingRemoved; listTitle = @"You unfollowed"; break;
        case ZUProfileAnalyzerMetricNotFollowingYouBack: users = d.notFollowingMeBack; listTitle = @"Not following you back"; break;
        case ZUProfileAnalyzerMetricYouDontFollowBack: users = d.youDontFollowBack; listTitle = @"You don't follow back"; break;
        case ZUProfileAnalyzerMetricMutualFollowers: users = d.mutualFollowers; listTitle = @"Mutual followers"; break;
        default: break;
    }
    if (!users.count) return;
    NSMutableSet<NSString *> *pinned = ZUProfileAnalyzerMutableSetForKey(kZUProfileAnalyzerPinnedPKsKey);
    NSMutableSet<NSString *> *ignored = ZUProfileAnalyzerMutableSetForKey(kZUProfileAnalyzerIgnoredPKsKey);
    BOOL shouldFilterIgnored = ((ZUProfileAnalyzerMetric)tag == ZUProfileAnalyzerMetricNotFollowingYouBack ||
                                (ZUProfileAnalyzerMetric)tag == ZUProfileAnalyzerMetricYouDontFollowBack);
    NSMutableArray<ZUProfileAnalyzerUser *> *filtered = [NSMutableArray array];
    for (ZUProfileAnalyzerUser *u in users) {
        if (shouldFilterIgnored && u.pk.length && [ignored containsObject:u.pk]) continue;
        [filtered addObject:u];
    }
    NSArray<ZUProfileAnalyzerUser *> *finalUsers = filtered;
    if (!finalUsers.count) return;
    NSArray<ZUProfileAnalyzerUser *> *sorted = [finalUsers sortedArrayUsingComparator:^NSComparisonResult(ZUProfileAnalyzerUser *a, ZUProfileAnalyzerUser *b) {
    ...
    ZUProfileAnalyzerListViewController *listVC = [[ZUProfileAnalyzerListViewController alloc] initWithTitle:listTitle users:sorted ?: @[]];
