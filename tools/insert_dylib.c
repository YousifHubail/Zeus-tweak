...
#define SWAP32(x, magic) (IS_LITTLE_ENDIAN(magic)? __builtin_bswap32(x): (x))
#define SWAP64(x, magic) (IS_LITTLE_ENDIAN(magic)? __builtin_bswap64(x): (x))
...
static int copy_file_data_unlink(const char *src_path, const char *dst_path) {
    int src_fd = -1;
    int dst_fd = -1;
    char buffer[64 * 1024];
    ssize_t bytes_read;

    /* Matches COPYFILE_UNLINK behavior */
    unlink(dst_path);
...
int main(int argc, const char *argv[]) {
	...
	if(!inplace_flag) {
		...
		if (copy_file_data_unlink(binary_path, new_binary_path) != 0) {
			printf("Failed to create %s: %s\n", new_binary_path, strerror(errno));
			exit(1);
		}
